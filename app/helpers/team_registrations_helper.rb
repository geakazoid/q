module TeamRegistrationsHelper
  ...
  def add_team_link(form_builder)
    link_to_function 'add another team' do |page|
      form_builder.fields_for :teams, Team.new, :child_index => 'NEW_RECORD' do |f|
        ...
        page << "$('teams').insert({ bottom: '#{escape_javascript(html)}'.replace(/NEW_RECORD/g, new Date().getTime()) });"
      end
    end
  end
  ...
  def remove_team_link(form_builder)
    if form_builder.object.new_record?
      ...
      link_to_function("remove", "$(this).up('.team').remove();");
    else
      ...
      link_to_function("remove", "$(this).up('.team').hide(); $(this).previous().value = '1'")
    end
  end
end